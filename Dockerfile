...
RUN apt-get update && apt-get install -y \
    git zip unzip wget curl python3-pip python3-dev build-essential \
    autoconf automake libtool pkg-config cmake libffi-dev libssl-dev \
    zlib1g-dev libjpeg-dev libpng-dev libncurses5-dev libncursesw5-dev \
    openjdk-17-jdk && rm -rf /var/lib/apt/lists/*

RUN pip3 install --upgrade pip && pip3 install buildozer cython

# 彻底禁用 buildozer 的 root 检查（避免 EOFError）
